...
fn u32_from_slice(buf: &[u8], offset: &mut usize) -> u32 {
    let mut v = 0;
    for _ in 0..4 {
        if *offset >= buf.len() {
            *offset = 0;
...
impl Blowfish {
    ...
    pub fn encrypt_vec(&self, data: &Vec<u8>) -> Vec<u8> {
        ...
        // NOTE(goat): copy_from_slice doesn't work here, since our out vector is bigger than data
        ...
        for i in (0..padded_len).step_by(8) {
            let xl = LE::read_u32(&out[i..i + 4]);
            let xr = LE::read_u32(&out[i + 4..i + 8]);

            let (nl, nr) = self.encrypt_pair(xl, xr);

            LE::write_u32(&mut out[i..i + 4], nl);
            LE::write_u32(&mut out[i + 4..i + 8], nr);
        }
    ...
    fn round_fun(&self, i: u32) -> u32 {
        // ((S[0, i >> 24] + S[1, (i >> 16) & 0xFF]) ^ S[2, (i >> 8) & 0xFF]) + S[3, i & 0xFF]
        let v1 = self.s[0][(i >> 24) as usize];
        let v2 = self.s[1][((i >> 16) & 0xFF) as usize];
        let v3 = self.s[2][((i >> 8) & 0xFF) as usize];
        let v4 = self.s[3][(i & 0xFF) as usize];

        (v1.wrapping_add(v2) ^ v3).wrapping_add(v4)
    }
